...
void main()
{
    ...
    cfe_uabipfe_fe_key_init(&fe_key, &uabipfe);
    err = cfe_uabipfe_derive_fe_key(&fe_key, &y, &master_sec_key, &uabipfe);

    cfe_uabipfe_ciphertext cipher;
    cfe_uabipfe_ciphertext_init(&cipher, &uabipfe);
    err = cfe_uabipfe_encrypt(&cipher, &master_pub_key, &x, &uabipfe);

    cfe_uabipfe decryptor;
    cfe_uabipfe_copy(&decryptor, &uabipfe);
    err = cfe_uabipfe_decrypt(xy, &cipher, &master_pub_key, &fe_key, &y, &decryptor);
